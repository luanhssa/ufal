...
void game(Game* jogo){
   ...
   while(!leave){
      ...
      if(ev.type == ALLEGRO_EVENT_TIMER) {
         ...
         if(getMouseX() >= (displayWidht*0.025622) && getMouseY() >= (displayHeigh*0.8723)
            ...
            //return 0;
                    ...
                    && playerONE && numCarta != 0 && !gamePaused) {
            char symbol, color;
            ...
            if(getColorTop(jogo) == color || getSymbolTop(jogo) == symbol || color == 'D'){
                ...
                //printf("%d  %c\n", getIdTop(jogo), getColorTop(jogo));
      ...
      if(redraw && al_is_event_queue_empty(event_queue)) {
         ...
         switch(getColorTop(jogo)){
            case 'Y':
                al_draw_bitmap(amarelo, 20, 60, 0);
            break;
            case 'B':
                al_draw_bitmap(azul, 20, 60, 0);
            break;
            case 'G':
                al_draw_bitmap(verde, 20, 60, 0);
            break;
            case 'R':
                al_draw_bitmap(vermelho, 20, 60, 0);
            break;
         }

         al_draw_text(fontPlayers, colorBlack, 60, 275, ALLEGRO_ALIGN_CENTRE, "Top Color");
