...
void descartar(Game* game, char symbol, char color){
    ...
    for(; i < getQTCards(jogador); i++) {
        if(symbol == peekSymbol(carta) && color == peekColor(carta)){
            ...
            if(jogador->hand == NULL){
                switch(getPlayerId(getJogadores(game))){
                    case 1:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Parabéns, Você ganhou!");
                        break;
                    case 2:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Luan ganhou!");
                        break;
                    case 3:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Carlos ganhou!");
                        break;
                    case 4:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Maxwell ganhou!");
                        break;
                    case 5:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Jorge ganhou!");
                        break;
                    case 6:
                        al_draw_text(fontMenu, colorBlack, (displayWidht/2),
                                     (displayHeigh/2), ALLEGRO_ALIGN_CENTRE, "Leonildo ganhou!");
                        break;
                }
                al_flip_display();
                al_rest(5.0);
