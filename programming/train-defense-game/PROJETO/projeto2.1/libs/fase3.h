...
int fase3_player1(void)
{
   ...
   ALLEGRO_DISPLAY *janela_fase3 = NULL;
   ...
   ALLEGRO_BITMAP *imagem_coqueiro = NULL;
   ...
   ALLEGRO_BITMAP *imagem_agua[2] = {NULL};
   ...
   janela_fase3 = al_create_display(SCREEN_X, SCREEN_Y);
   if(!janela_fase3) {
      fprintf(stderr, "failed to create janela_fase3!\n");
   ...
   if(!imagem_player1) {
      ...
      al_destroy_display(janela_fase3);
      al_destroy_timer(timer);
      return -1;
   }
   ...
   al_set_target_bitmap(al_get_backbuffer(janela_fase3));
   ...
   al_register_event_source(event_queue, al_get_display_event_source(janela_fase3));
   ...
   musica_fundo = al_load_sample( "audio/footstep3.wav" );
   som_tiro = al_load_sample("audio/tiro.wav");
   som_boss = al_load_sample("audio/tiro_boss.wav");

/******************************* LOAD IMAGES ***********************************************/

   imagem_fundo_fase1 = al_load_bitmap("imagens/fundo_game_3.png");
   imagem_ceu=al_load_bitmap("imagens/ceu_game_1.png");
   imagem_agua[0] = al_load_bitmap("imagens/agua1.png");
   imagem_agua[1] = al_load_bitmap("imagens/agua2.png");
   ...
   imagem_coqueiro = al_load_bitmap("imagens/coqueiro.png");
...
/************************************************************************************////
        ...
        //////////////////////////////////////////////////////////////////////
                 ...
                 for(i=0;i<12;i++) /// DEIXA INATIVO OS coqueiroS QUE SAIRAM DA TELA
                 {
                    if(coqueiro[i].estado == EST_ATIVO)
                    {
                       if(coqueiro[i].x<-20)
                       {
                          coqueiro[i].estado==EST_INATIVO;
                 ...
                 if(cont_tempo%1==0 && estacao.estado==EST_INATIVO)
                 {
                    //for(i=0;i<5;i++)
                    //{
                       MEXESOLTA(1);  /// MEXE OS COQUEIROS
                    //}
                 }
                 if(cont_tempo%200==0 && estacao.estado==EST_INATIVO)   /// SOLTADOR DE COQUEIROS
                 {
                    for(i=0;i<12;i++)
                    {
                       if(i<11)
                       {	srand ( time(NULL) );
                          if (coqueiro[i].estado==EST_INATIVO && rand()%2 ==0)
                          {
                              coqueiro[i].x=soltaObjetos[1].x;
                              coqueiro[i].y=soltaObjetos[1].y;
                              coqueiro[i].estado=EST_ATIVO;
                              i = 12;
                          }
                       }
                    }
...
/// ----------------------------- COMEÇA A DESENHAR --------------------------------------------- ///
         ...
         al_draw_bitmap(imagem_agua[cont_agua],0,0,0);
...
////////////////// COQUEIRO
         for(i=0;i<12;i++)
         {
            if(coqueiro[i].estado==EST_ATIVO)
            {
               al_draw_bitmap(imagem_coqueiro,coqueiro[i].x,coqueiro[i].y,0); /// desenha os coqueiros ativos
            }
         }
////////////////// END COQUEIRO
////////////////// FASE 1
         ...
         {
            al_draw_textf(font_fase,al_map_rgb(0,0,0),640,360,ALLEGRO_ALIGN_LEFT,"3");
         }
////////////////// END FASE 1
////////////////// imagem_vidas
