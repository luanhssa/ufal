...
int fase2_player1(void)
{
   ...
   ALLEGRO_DISPLAY *janela_fase1 = NULL;
   ...
   ALLEGRO_BITMAP *imagem_feno[8] = {NULL};
   ...
   janela_fase1 = al_create_display(SCREEN_X, SCREEN_Y);
   if(!janela_fase1) {
      fprintf(stderr, "failed to create janela_fase1!\n");
   ...
   if(!imagem_player1) {
      ...
      al_destroy_display(janela_fase1);
      al_destroy_timer(timer);
      return -1;
   }
   ...
   al_set_target_bitmap(al_get_backbuffer(janela_fase1));
   ...
   al_register_event_source(event_queue, al_get_display_event_source(janela_fase1));
   ...
   musica_fundo = al_load_sample( "audio/footstep.wav" );
   som_tiro = al_load_sample("audio/tiro.wav");
   som_boss = al_load_sample("audio/tiro_boss.wav");

/******************************* LOAD IMAGES ***********************************************/

   imagem_fundo_fase1 = al_load_bitmap("imagens/fundo_game_1.png");
   ...
   imagem_feno[0] = al_load_bitmap("imagens/boladefeno1.png");
   imagem_feno[1] = al_load_bitmap("imagens/boladefeno2.png");
   imagem_feno[2] = al_load_bitmap("imagens/boladefeno3.png");
   imagem_feno[3] = al_load_bitmap("imagens/boladefeno4.png");
   imagem_feno[4] = al_load_bitmap("imagens/boladefeno5.png");
   imagem_feno[5] = al_load_bitmap("imagens/boladefeno6.png");
   imagem_feno[6] = al_load_bitmap("imagens/boladefeno7.png");
   imagem_feno[7] = al_load_bitmap("imagens/boladefeno8.png");
...
/************************************************************************************////
                 ...
                 {
                    if(estacao.x>100 && estacao.estado==EST_ATIVO)
                    {
                       estacao.x-=2;
                    }
                 }
        //////////////////////////////////////////////////////////////////////
                 ...
                 if(cont_tempo<1800)/*10800)*/ /// vai se mexer pra cima do personagem enquanto o tempo for menor que 3min
                 {
                 ...
                 if(boss.estado==EST_ATIVO) /// movimento do boss
                 {
        ...
        //////////////////////////////////////////////////////////////////////
                 ...
                 for(i=0;i<12;i++) /// DEIXA INATIVO OS FENOS QUE SAIRAM DA TELA
                 {
                    if(feno[i].estado == EST_ATIVO)
                    {
                       if(feno[i].x<-20)
                       {
                          feno[i].estado==EST_INATIVO;
                 ...
                 if(cont_tempo%1==0 && estacao.estado==EST_INATIVO)
                 {
                    //for(i=0;i<5;i++)
                    //{
                       MEXESOLTA(1);  /// MEXE OS FENOS
                    //}
                 }
                 if(cont_tempo%500==0 && estacao.estado==EST_INATIVO)   /// SOLTADOR DE FENOS
                 {
                    for(i=0;i<12;i++)
                    {
                       if(i<11)
                       {	srand ( time(NULL) );
                          if (feno[i].estado==EST_INATIVO && rand()%2 ==0)
                          {
                              feno[i].x=soltaObjetos[1].x;
                              feno[i].y=soltaObjetos[1].y;
                              feno[i].estado=EST_ATIVO;
                              i = 12;
                          }
                       }
                    }
                 }
                 if(cont_tempo%200==0 && cont_tempo<1800)/*10800)*/   /// soltador de inimigos só solta qndo for menor que 3min.
                 {
                    if(inimigo_delay==0)
                    {
                       for(i=0;i<10;i++)
                 ...
                 if(cont_tempo>2000/*10800*/ && boss.estado==EST_INATIVO && BOSS_DEAD==EST_ATIVO)
                 {
                    estacao.estado = EST_ATIVO;
                 }
                 if(cont_tempo>1000 && estacao.estado==EST_INATIVO)//10800)
                 {
                    for(i=0;i<10;i++)
                    {
...
////////////////// FENO
         for(i=0;i<12;i++)
         {
            if(feno[i].estado==EST_ATIVO)
            {
               al_draw_bitmap(imagem_feno[cont_trem],feno[i].x,feno[i].y,0); /// desenha os fenos ativos
            }
         }
////////////////// END FENO
////////////////// FASE 1
         ...
         {
            al_draw_textf(font_fase,al_map_rgb(0,0,0),640,360,ALLEGRO_ALIGN_LEFT,"1");
         }
////////////////// END FASE 1
////////////////// imagem_vidas
...
//////////////// NEXT LEVEL
         if(estacao.x<600)
         {
            al_draw_textf(font_gameover, al_map_rgb(0,0,0), 750, 350,ALLEGRO_ALIGN_RIGHT,
